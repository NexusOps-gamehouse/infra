 ...
 *
 * mode 인자는 없다. scale.json 을 갈아끼우면 SEED 가 그 프로파일 값이 되므로
 * 스모크든 회차든 같은 방식으로 대조하면 된다.
 *   SCALE=smoke k6 run k6/rounds/smoke.js
 */
export function assertSeed() {
  const problems = [];

  // 프로파일이 어긋나면 나머지는 볼 것도 없다.
  // data.example 을 물고 회차를 도는 사고가 여기서 걸린다.
  if (meta.profile !== SEED.profile) {
    problems.push(
      `프로파일 '${meta.profile}' ≠ 기대 '${SEED.profile}' ` +
      `— 다른 규모로 시딩된 데이터다`,
    );
  }
 ...
 * 결함이 아니라 정상적인 비즈니스 거절이라, 시나리오 4 만 400 을 정상으로 본다.
 * (다른 시나리오의 4xx 는 실패다 — config.js 의 expect 참조)
